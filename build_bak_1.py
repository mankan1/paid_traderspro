...
# ─────────────────────────────────────────────────────────────────
NAV_INJECT = """\
  <!-- Google user badge + premium status (populated by /me) -->
  <div id="oracleUserBadge" style="display:none;align-items:center;gap:10px;margin-left:auto;padding:0 16px;flex-shrink:0">
    <span id="premiumBadgeNav" style="display:none;font-family:var(--font-mono);font-size:8px;
          ...
          padding:2px 8px;border-radius:2px;background:rgba(240,192,64,.08)">⚡ PRO</span>
    <img id="oracleAvatar" src="" width="26" height="26"
         style="border-radius:50%;border:1px solid rgba(255,255,255,.15)" />
    <span id="oracleUserName" style="font-family:var(--font-mono);font-size:10px;color:var(--t2)"></span>
    <button id="manageSubBtn" onclick="openManageSub()" style="display:none;
            ...
            background:rgba(255,255,255,.04)">MANAGE SUB</button>
    <a href="/logout" style="font-family:var(--font-mono);font-size:9px;color:var(--t3);
       ...
       background:rgba(255,255,255,.04)">LOGOUT</a>
  </div>
</nav>"""
...
<div id="paywallOverlay">
  <div id="paywallModal">
    ...
    <div class="pm-price">$10<span style="font-size:16px;color:var(--t3)">/mo</span></div>
...
(function() {
  ...
  async function initPaywall() {
    ...
    if (_isPremium) {
      ...
      if (manageBtn) manageBtn.style.display  = 'inline';
  ...
  window.openManageSub = async function() {
    try {
      const r = await fetch('/stripe/portal', { method:'POST', credentials:'same-origin' });
      const { url } = await r.json();
      window.location.href = url;
    } catch(e) { alert('Error opening billing portal: ' + e.message); }
  };
